...
class UpdatePlayCount {
    ...
    public static function playCount($playId): string
    {
        $playButton = "<a href='?id={$playId}&playSong={$playId}' class='hover:text-slate-500 hover:cursor-pointer'>
    ...
    public function updateDatabase(int $playId): bool
    {
        $query = $this->db->prepare('UPDATE `songs`
                                            SET `play_count` = `play_count` + 1
                                            WHERE `songs`.`id` = :songId');
        return $query->execute(['songId' => $playId]);
